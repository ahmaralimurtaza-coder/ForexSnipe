...
class ApiService {
  ...
  static const _myfxbookEmail     = String.fromEnvironment('MYFXBOOK_EMAIL');
  static const _myfxbookPassword  = String.fromEnvironment('MYFXBOOK_PASSWORD');
  ...
  Future<String?> loginMyfxbook() async {
    try {
      final email = Uri.encodeComponent(_myfxbookEmail);
      final pass  = Uri.encodeComponent(_myfxbookPassword);
      final url = 'https://www.myfxbook.com/api/login.json?email=$email&password=$pass';
      final res = await _client.get(Uri.parse(url), headers: _h).timeout(const Duration(seconds: 15));
      if (res.statusCode == 200) {
        final data = json.decode(res.body);
    ...
    } catch (e) { print('MyfxbookLogin: $e'); }
    return null;
  }

  Future<http.Response> fetchMyfxbookSentiment(String session) async {
    final url = 'https://www.myfxbook.com/api/get-community-outlook.json?session=$session';
    return await _client.get(Uri.parse(url), headers: _h).timeout(const Duration(seconds: 15));
  }
