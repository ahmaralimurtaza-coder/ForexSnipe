...
class _AiScreenState extends State<AiScreen> {
  ...
  String _getResponse(String q) {
    ...
    if (t.contains('limitation') || t.contains('delay') || t.contains('accuracy') || t.contains('drawback') || t.contains('real time') || t.contains('realtime') || t.contains('live data') || t.contains('data source') || t.contains('how accurate') || t.contains('app info')) {
      return 'ForexSnipe Data Sources & Limitations:\n\nPRICES TAB\n- Forex: 50 pairs, live via CurrencyFreaks/Frankfurter cascade\n- Crypto: 30 coins, real-time via Binance\n- Gold/Silver/Platinum: Real-time via Yahoo Finance futures\n- Stocks: 30 tickers, Indices: 20, Commodities: 20, Futures: 20 - all via Yahoo Finance\n\nCOT TAB\n- Data from CFTC.gov, released every Friday, 1 week behind by design\n- Covers 11 major/exotic currencies and 15 commodities where a CFTC report legally exists\n- Individual stocks, altcoins, and non-US indices have no COT report - this is a market structure fact, not a bug\n\nOPEN INTEREST TAB\n- Same official CFTC dataset as the COT tab, sorted by highest open interest\n- Covers only instruments with an actual CFTC-tracked futures contract\n\nNEWS TAB\n- Auto-refreshes every 5 minutes, 100+ keyword detection tags articles to specific pairs\n- Sources: Reuters, Bloomberg, CNBC, Finnhub\n\nCALENDAR TAB\n- Finnhub API, refreshes every 15 minutes\n\nSENTIMENT TAB\n- Live retail trader sentiment via Myfxbook API (real broker data) where covered\n- Falls back to CFTC COT-derived or price momentum data where Myfxbook has no coverage\n- Refreshes every 20 minutes\n\nSOURCES TAB (Barchart Analysis)\n- Barchart Trader Cheat Sheet embedded\n- S/R levels, Pivots, Fibonacci, Moving Averages\n- Updated daily from Barchart.com\n\nQUIZ TAB\n- 170 questions, fully offline, no network needed\n- Easy (20), Medium (50), Hard (100) difficulty levels\n\nLOT CALC TAB\n- Calculates position size from account balance, risk %, and stop loss (pips or entry/SL price)\n- Covers Forex and Commodities (Gold, Silver, Copper, Platinum, Palladium, etc)\n- Auto mode uses this app\'s live prices; Manual mode lets you type your own pip value with zero API dependency\n\nWARNING: Use ForexSnipe for context only. Always verify on TradingView before trading.';
    }
    ...
    if (t.contains('lot size') || t.contains('lot ')) {
      return 'Lot Sizes in Forex:\n\n• Standard Lot = 100,000 units of base currency\n• Mini Lot = 10,000 units\n• Micro Lot = 1,000 units\n• Nano Lot = 100 units (offered by some brokers)\n• Smaller lot sizes allow finer position sizing for proper risk management\n• Beginners should generally start with micro or mini lots\n\nUse the Lot Calc tab in this app to compute your exact position size - enter your balance, risk %, and stop loss (in pips or as an entry/SL price range), and it calculates lots for you automatically using live prices, or you can enter a pip value manually.';
    }
